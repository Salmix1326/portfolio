...
***************************************************************************** */
        ...
        function adaptMenu() {
            ...
            if (window.innerWidth <= 767.98) {
                todayBtn.innerHTML = `1d`;
                weekBtn.innerHTML = `7d`;
                monthBtn.innerHTML = `30d`;
            }
        }
        adaptMenu();
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            function countdownTimer() {
                ...
                $hours.dataset.title = declensionNum(hours, [ "Hours", "Hours", "Hour" ]);
        ...
        class NMasonryGrid {
            constructor(element, options) {
                const settings = {
                    ...
                    gapColumn: 20,
                    gapRow: 20,
                    gridInitClassName: "n_masonry-grid",
                    itemClassName: "adaptive-grid-item",
                    itemActiveClassName: "adaptive-filter-item-active",
                    itemHiddenClassName: "adaptive-filter-item-hidden",
                ...
                this.containerGrid = element;
                this.childrenNodes = element.children;
                this.childrenData = Array.from(this.childrenNodes).map((childNode => ({
                    childNode
                })));
                this.containerGrid.classList.add(this.settings.gridInitClassName);
                this.childrenData.forEach((child => child.childNode.classList.add(this.settings.itemClassName)));
                this.settingsSizes = Object.keys(this.settings.responive).map((item => Number(item))).sort(((a, b) => b - a));
                this.init();
            }
            init() {
                document.fonts.ready.then((() => {
                    this.containerGrid.classList.add("_grid-init_");
                    if (this.settings.filter) this.filterInit();
                    this.setEvents();
                    this.setParaneters();
                }));
            }
            destroy() {
                this.containerGrid.classList.remove("_grid-init_");
                window.removeEventListener("resize", this.debouncedResize);
                this.observer.disconnect();
                this.removeParemeters();
                if (this.settings.filter) {
                    this.containerGrid.classList.remove("_filter-init_");
                    this.filterNavigation.classList.remove("_filter-init_");
                    this.filterNavigation.removeEventListener("click", this.clickFilter);
                }
            }
            setEvents() {
                this.debouncedResize = this.debounce(this.setParaneters.bind(this));
            ...
            setMutation() {
                ...
                this.observer.observe(this.containerGrid, mutation);
            }
            callbackMutation=mutationsList => {
                for (let mutation of mutationsList) if ("attributes" == mutation.type && "class" == mutation.attributeName || "childList" == mutation.type || "characterData" == mutation.type) {
                    this.childrenNodes = this.containerGrid.children;
                    this.childrenData = Array.from(this.childrenNodes).map((childNode => ({
                        childNode
                    })));
                    this.setParaneters();
            ...
            setParaneters() {
                const containerWidth = this.containerGrid.clientWidth;
                this.setCurrentSettings(document.documentElement.clientWidth);
                const widthItem = (containerWidth - this.gapColumn * (this.columns - 1)) / this.columns;
                this.setWidth(widthItem);
                this.childrenData = this.childrenData.map((child => {
                    child.childNode.style.removeProperty("height");
                    return {
                        ...child,
            ...
            setCurrentSettings(windowWidth) {
                const currentResponsiveKey = this.settingsSizes.find((size => windowWidth >= size));
                this.columns = currentResponsiveKey ? this.settings.responive[currentResponsiveKey].columns : this.settings.columns;
                this.gapColumn = currentResponsiveKey ? this.settings.responive[currentResponsiveKey].gapColumn : this.settings.gapColumn;
                this.gapRow = currentResponsiveKey ? this.settings.responive[currentResponsiveKey].gapRow : this.settings.gapRow;
            }
            setWidth(widthItem) {
                this.childrenData.forEach((child => child.childNode.style.width = `${widthItem}px`));
            }
            setSizeContainer() {
                let heightColumns = new Array(this.columns).fill(0);
                let heightitem = new Array(this.columns).fill(0);
                const visibilityElement = [];
                this.childrenData.forEach((el => {
                    if (this.settings.filter) {
                        const child = el.childNode;
                        if (child.classList.contains(this.settings.itemActiveClassName)) visibilityElement.push(el);
                    } else visibilityElement.push(el);
                }));
                let cnt = 0;
                visibilityElement.forEach(((child, i, arr) => {
                    if (!this.settings.grid) {
                        heightitem[i % this.columns] = child.height;
                        heightColumns[i % this.columns] += child.height + this.gapRow;
                    } else {
                        heightitem[i % this.columns] = child.height;
                        if ((i + 1) % this.columns == 0 || i == arr.length - 1) {
                            const max = Math.max(...heightitem);
                            for (let k = 0; k < heightColumns.length; k++) {
                                heightitem[k] = 0;
                                heightColumns[k] += max + this.gapRow;
                            }
                            if (1 != this.columns) for (let j = cnt; j < this.columns; j++) {
                                if (!arr[j]) break;
                                arr[j].childNode.style.height += `${max}px`;
                                arr[j].height = max;
                                cnt++;
                            }
                        }
                ...
                const maxHeightColumn = Math.max(...heightColumns);
                const height = visibilityElement.length ? maxHeightColumn - this.gapRow : 0;
                this.containerGrid.style.height = `${height}px`;
            }
            setPosition(widthItem) {
                const topSets = new Array(this.columns).fill(0);
                let cnt = 0;
                this.childrenData = this.childrenData.map(((child, i) => {
                    const indexColumn = cnt % this.columns;
                    const left = indexColumn * widthItem + this.gapColumn * indexColumn;
                    const top = topSets[indexColumn];
                    if (!child.childNode.classList.contains(this.settings.itemHiddenClassName)) {
                        topSets[indexColumn] += child.height + this.gapRow;
                        cnt += 1;
                    }
                    return {
                        ...child,
                ...
                this.childrenData.forEach((child => {
                    child.childNode.style.setProperty("--x", `${child.left}px`);
                    child.childNode.style.setProperty("--y", `${child.top}px`);
                }));
            }
            removeParemeters() {
                this.childrenData.forEach((child => {
                    child.childNode.removeAttribute("data-sort-filter");
                    child.childNode.removeAttribute("style");
                }));
                this.containerGrid.style.height = "";
            }
            filterInit() {
                this.filterId = this.containerGrid.dataset.filterBody;
                this.filterNavigation = document.querySelector(`[data-filter-navigation="${this.filterId}"]`);
                this.childrenData.forEach((el => {
                    const card = el.childNode;
                    card.classList.add(this.settings.itemActiveClassName);
                }));
                const go = this.containerGrid.hasAttribute("data-filter-body");
                if (!go) return;
                this.containerGrid.classList.add("_filter-init_");
                if (this.filterNavigation) {
                    this.filterNavigation.classList.add("_filter-init_");
                    this.fiterButtons = this.filterNavigation.querySelectorAll("[data-filter-button]");
                    let filterId = [];
                    this.fiterButtons.forEach((button => {
                        if (button.classList.contains(this.settings.buttonActiveClassName)) {
                            const idInButton = button.dataset.filterButton ?? "";
                            if (!this.settings.filterMultiple && "" == filterId) {
                                filterId = idInButton;
                                this.setActiveCards(filterId);
                            } else if (this.settings.filterMultiple) filterId.push(idInButton);
                        }
                    }));
                    if (this.settings.filterMultiple) this.setActiveCards(filterId);
                    this.setClickEvent();
                }
            }
            setClickEvent() {
                this.clickFilter = this.filterEventClick.bind(this);
                this.filterNavigation.addEventListener("click", this.clickFilter);
            }
            filterEventClick(e) {
                e.preventDefault();
                const button = e.target.closest("[data-filter-button]");
                if (!button) return;
                const isAll = "" == button.getAttribute("data-filter-button") || "all" == this.formatValue(button.getAttribute("data-filter-button"));
                if (!this.settings.filterMultiple || isAll) this.fiterButtons.forEach((button => {
                    button.classList.remove(this.settings.buttonActiveClassName);
                }));
                if (!isAll) button.classList.toggle(this.settings.buttonActiveClassName);
                const currentValue = this.settings.filterMultiple ? this.getValueButtons() : button.getAttribute("data-filter-button");
                this.setActiveCards(currentValue);
            }
            setActiveCards(values) {
                const currentValue = Array.isArray(values) ? values.map((value => this.formatValue(value.toString()) ? this.formatValue(value.toString()) : "all")) : values ? values.replace(/\s/g, "").toLowerCase().split(",") : [ "all" ];
                this.childrenData.forEach((el => {
                    const card = el.childNode;
                    const valuesCard = this.formatValue(card.dataset.filterCard);
                    const arrValuesCard = currentValue.includes("all") ? [ "all" ] : valuesCard.split(",");
                    if (arrValuesCard.some((el => currentValue.includes(el)))) {
                        card.classList.add(this.settings.itemActiveClassName);
                        card.classList.remove(this.settings.itemHiddenClassName);
                    } else {
                        card.classList.remove(this.settings.itemActiveClassName);
                        card.classList.add(this.settings.itemHiddenClassName);
                    }
                }));
            }
            thereIsAnEmpty() {
                const activeButton = this.filterNavigation.querySelectorAll(`.${this.settings.buttonActiveClassName}`);
                return activeButton.length;
            }
            getValueButtons() {
                if (!this.thereIsAnEmpty()) return "all";
                let values = [];
                this.fiterButtons.forEach((button => {
                    if (button.classList.contains(this.settings.buttonActiveClassName)) {
                        const arrValues = this.formatValue(button.getAttribute("data-filter-button")).split(",");
                        if (!values.some((el => arrValues.includes(el)))) arrValues.forEach((value => values.push(value)));
                    }
                }));
                return values;
            }
        ...
        const masonrysGrid = document.querySelectorAll("[data-masonry]");
        if (masonrysGrid.length) masonrysGrid.forEach((masonryGrid => {
            new NMasonryGrid(masonryGrid, {
                filter: true,
                responive: {
                    1: {
                        columns: 1,
